...
class AppStore extends Component {
  state = {displayCategoryId: 'SOCIAL', searchInput: ''}

  changeTabUIFunction = tabId => {
    this.setState({displayCategoryId: tabId})
  }
  ...
  render() {
    const {displayCategoryId, searchInput} = this.state

    const filteredAppListAsPerCategory = appsList.filter(
      eachApp => eachApp.category === displayCategoryId,
    )

    const searchInputIncluded = filteredAppListAsPerCategory.filter(eachItem =>
    ...
    return (
      <div className="complete-bg-container">
        ...
        <ul className="application-tab">
          {tabsList.map(eachTab => (
            <TabItem
              changeTabUIFunction={this.changeTabUIFunction}
              tabDetail={eachTab}
              key={eachTab.tabId}
              isActiveTab={eachTab.tabId === displayCategoryId}
            />
          ))}
        </ul>
        <ul className="application-container">
          {searchInputIncluded.map(eachApp => (
